...
import { addfournisseur,updatefournisseur } from './fournisseurService';
...
export const Fournisseur = ({ recordForEdit, addOrEdit, onClose }) => {
    ...
    const handlefournisseurTypeChange = (event) => {
    ...
    const validate = () => {
        ...
        if (!formData.typeC) tempErrors.typeC = "Veuillez sélectionner un type de fournisseur.";
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            ...
            if (recordForEdit) {
                await updatefournisseur(cref, formData);
                alert('fournisseur updated successfully!');
            } else {
                await addfournisseur(dataToSend);
            ...
            <Card>
                <CardContent>
                    <Typography variant="h6" component="div" sx={{ mb: 2 }}>
                        {recordForEdit ? 'Edit fournisseur' : 'Nouveau(-elle) fournisseur(e)'}
                    </Typography>
                    <FormControl component="fieldset" error={Boolean(errors.typeC)}>
                        <FormLabel component="legend">Type de fournisseur</FormLabel>
                        <RadioGroup
                            row
                            name="typeC"
                            value={formData.typeC}
                            onChange={handlefournisseurTypeChange}
                        >
                            <FormControlLabel 
                ...
                <CardActions>
                    <Button 
                        ...
                        fullWidth
                    >
                        {recordForEdit ? 'Modifier' : 'Ajouter'} fournisseur
                    </Button>
                    <Button 
                        variant="outlined" 
                        color="secondary" 
